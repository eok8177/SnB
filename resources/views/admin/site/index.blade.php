      ...
      @foreach($blocks as $block)
        <li data-id="{{$block->id}}" class="list-group-item d-flex justify-content-between align-items-center">
          ...
          <span class="d-flex align-items-center">
            ...
            <a href="{{route('admin.site.edit', $block)}}" class="btn d-flex align-items-center">
          ...
          <span class="btns text-nowrap">
            <a href="{{route('admin.toggle-show',["id"=>$block->id,"model"=>"Site"])}}" class="btn change-status"title="Toggle Active">
              <i class="fa fa-{{$block->show ? 'eye' : 'eye-slash'}}"></i>
            </a>
            <a href="{{route('admin.site.edit', $block)}}" class="btn ms-1"><i class="fa fa-pencil" aria-hidden="true"></i></a>
            <a href="{{route('admin.site.destroy',$block)}}" class="btn ms-1 delete"><i class="fa fa-trash-o" aria-hidden="true"></i></a>
          </span>
...
<div class="modal fade" id="addModal" tabindex="-1" aria-labelledby="addModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <form action="{{route('admin.site.create')}}" method="POST">
  ...
  $('.sorted-list').sortable({
    ...
    onDrop: function ($item, container, _super) {
      ...
      $.ajax({
          ...
          type: 'PUT',
          dataType: 'json',
          url: '{{route("admin.change-order",["model"=>"Site"])}}'
      });
